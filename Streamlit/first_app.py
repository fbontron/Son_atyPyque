...
st.title('Détection de sons atypiques')
...
classification=st.sidebar.checkbox('Modèle de classification')
...
if intro:
    ...
    st.markdown("On sent qu'n perd la dépendance temporelle, qu'il serait possible de retrouver en introduisant des neurones récurrents dans le réseau") 
    ...
    st.markdown("L'objectif est de prédire si le spétrogramme en entrée de notre modèle appartient à une pompe normal ou anormal.")
    ...
    st.markdown("Architecture du réseau CNN")
    ...
    #st.markdown("Pour notre modèle de classification nous considérons comme sons normaux les données sonores issues du fichier pump/train ⇒ label 0.")
    #st.markdown("En parallèle les données sonores issues des autres datasets d'entraînement (fan, valve et slider) sont considérées comme sons anormaux ⇒ label 1.")   

    accuracy = 0.71 
    
    result_list = ["confusion matrix", "accuracy", "ROC curve"]

    selected_result = st.selectbox("Résultat", result_list)
